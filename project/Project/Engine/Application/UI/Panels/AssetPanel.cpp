...
void AssetPanel::DrawToolbar() {
	...
	if (typeFilter_.has_value()) {
		...
		switch (*typeFilter_) {
			case AssetType::Texture:  tname = "Texture";  break;
			case AssetType::Model:    tname = "Model";    break;
			case AssetType::Shader:   tname = "Shader";   break;
			case AssetType::Material: tname = "Material"; break;
			case AssetType::Audio:    tname = "Audio";    break;
			default: break;
		}
...
void AssetPanel::DrawFavorites() {
	if (ImGui::TreeNodeEx("Favorites", ImGuiTreeNodeFlags_DefaultOpen | ImGuiTreeNodeFlags_SpanFullWidth)) {
		if (ImGui::Selectable("All Textures")) { typeFilter_ = AssetType::Texture;  scope_ = Scope::All; }
		if (ImGui::Selectable("All Models")) { typeFilter_ = AssetType::Model;    scope_ = Scope::All; }
		if (ImGui::Selectable("All Shaders")) { typeFilter_ = AssetType::Shader;   scope_ = Scope::All; }
		if (ImGui::Selectable("All Audio")) { typeFilter_ = AssetType::Audio;    scope_ = Scope::All; }
		if (ImGui::Selectable("All Materials")) { typeFilter_ = AssetType::Material; scope_ = Scope::All; }
		ImGui::TreePop();
	}
}
...
void AssetPanel::DrawRightView() {
	...
	if (!gridMode_) {
		...
		while (clip.Step()) {
			for (int i = clip.DisplayStart; i < clip.DisplayEnd; ++i) {
				...
				if (thumb) ImGui::ImageButton("##thumb", thumb, ImVec2(20, 20));
				else       ImGui::Button("No Preview", ImVec2(20, 20));
	...
	while (clip.Step()) {
		...
		for (int i = startIdx; i < endIdx; ++i) {
			...
			if (thumb) ImGui::ImageButton("##thumb", thumb, sz);
			else       ImGui::Button("No Preview", sz);
