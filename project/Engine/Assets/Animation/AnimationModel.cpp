...
namespace CalyxAssets {
	...
	void AnimationModel::AddAnimation(const std::string& name,const std::string& file) {
		AnimationState st;
		st.name      = name;
		st.animation = LoadAnimationFile("Resources/models",file);
	...
	void AnimationModel::SkeletonUpdate() {
		...
		for(Joint& joint : modelData_->skeleton.joints) {
			...
			if(joint.parent) { joint.skeletonSpaceMatrix = joint.localMatrix * modelData_->skeleton.joints[*joint.parent].skeletonSpaceMatrix; } else { joint.skeletonSpaceMatrix = joint.localMatrix; }
		}
	}
	...
	void AnimationModel::OnModelLoaded() {
		...
		for(auto& j : modelData_->skeleton.joints) { j.restTransform = j.transform; }
		/* 高速テーブル */
		/* --- ここで全アニメの fastChannels を作る ------------------- */
		for(auto& [_, state] : animationStates_) { BuildFastChannels(state.animation); }
	}
	...
	void AnimationModel::ShowImGuiInterface() {
		...
		if(ImGui::CollapsingHeader("Skeleton##header")) {
			...
			if(jointNames.empty() && modelData_) {
				...
				for(auto& j : modelData_->skeleton.joints) { jointNames.push_back(j.name.c_str()); }
			}
	...
	std::vector<std::string> AnimationModel::GetAnimationNodeNames() const {
		...
		for(auto& pair : modelData_->animation.nodeAnimations) { names.push_back(pair.first); }
		return names;
	}
