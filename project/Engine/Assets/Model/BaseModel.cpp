...
void BaseModel::ShowImGuiInterface() {
	...
	if(GuiCmd::CollapsingHeader("Draw")) {
		...
		if(GuiCmd::Combo("Blend Mode",currentBlendMode,blendModeNames, IM_ARRAYSIZE(blendModeNames))) { blendMode_ = static_cast<BlendMode>(currentBlendMode); }
	}
}
...
void BaseModel::ApplyConfig(const BaseModelConfig& config) {
	...
	//  GUID があれば最優先
	if(config.textureGuid.isValid()) { ok = LoadTextureByGuid(config.textureGuid); }
...
void BaseModel::ShowImGui(BaseModelConfig& config) {
	...
	if (ImGui::TreeNodeEx("Texture (Drag & Drop from Assets)", ImGuiTreeNodeFlags_SpanAvailWidth | ImGuiTreeNodeFlags_DefaultOpen)) {
		...
		if(ImGui::BeginDragDropTarget()) {
			if(const ImGuiPayload* p = ImGui::AcceptDragDropPayload("CALYX_ASSET")) {
				...
				if(payload.type == AssetType::Texture) {
					...
					} else { ImGui::OpenPopup("TextureDropError"); }
				}
			}
		...
		auto labelFromGuid = [](const Guid& g)-> std::string {
			...
			auto* db = AssetDatabase::GetInstance();
			for(auto* r : db->GetView()) { if(r && r->type == AssetType::Texture && r->guid == g) { return r->sourcePath.filename().string(); } }
			return "(missing)";
		};
		ImGui::TextDisabled("Current: %s",labelFromGuid(textureGuid_).c_str());
		ImGui::SameLine();
		if(textureGuid_.isValid() && ImGui::SmallButton("Copy GUID")) { ImGui::SetClipboardText(textureGuid_.ToString().c_str()); }
