...
public:
	...
	// 初期化（基準位置・1桁サイズ）
	void Initialize(const CxMath::Vector2& pos, const CxMath::Vector2& digitSize);
	...
	// レイアウト設定
	void SetPosition(const CxMath::Vector2& pos);
	void SetDigitSize(const CxMath::Vector2& size);
	void SetSpacing(float px);
	void SetAlign(DigitsAlign a);
	void SetAnchor(const CxMath::Vector2& anc);
...
private:
	...
	std::string dir_;
	std::string ext_;
	CxMath::Vector2		origin_{0, 0};
	CxMath::Vector2		digitSize_{32, 48};
	CxMath::Vector2		anchor_{0.5f, 0.5f};
	float		spacing_   = 2.0f;
