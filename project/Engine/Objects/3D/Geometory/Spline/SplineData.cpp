...
void SplineData::SegmentAndLocalT(float t, int& seg, float& lt) const {
	...
	if (segCount <= 0) { seg = 0; lt = 0.0f; return; }
...
Vector3 SplineData::CatmullRom(const Vector3& p0, const Vector3& p1, const Vector3& p2, const Vector3& p3, float t) {
...
Vector3 SplineData::CatmullRomTangent(const Vector3& p0, const Vector3& p1, const Vector3& p2, const Vector3& p3, float t) {
	float t2 = t * t;
	// d/dt
	return 0.5f * (
		(-p0 + p2) +
...
Vector3 SplineData::Tangent(float t) const {
	const int n = CountP();
	if (n <= 1) return Vector3(0, 0, 1);
	...
	Vector3 tg = CatmullRomTangent(points[i0].pos, points[i1].pos, points[i2].pos, points[i3].pos, lt);
...
float SplineData::AdvanceTBy(float t, float distance) const {
	...
	// まず t を LUT 上で位置づけ
	float tClamped = t;
	if (!closed) tClamped = std::clamp(tClamped, 0.0f, 1.0f);
	else tClamped = std::fmod(std::fmod(tClamped, 1.0f) + 1.0f, 1.0f);
