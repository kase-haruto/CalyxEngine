...
DirectionalLight::DirectionalLight(const std::string& name) {
	...
	lightData_.direction = CxMath::Vector3(-0.08f,-1.0f,0.34f);  // ライトの向き
...
void DirectionalLight::DrawDebug() {
	...
	// ライトの始点（ワールド座標系での位置）
	const CxMath::Vector3 start = worldTransform_.GetWorldPosition();

	// ライトの向き（方向ベクトル × 長さ）
	const CxMath::Vector3 dir    = lightData_.direction.Normalize();
	const float   length = 3.0f; // 可視化用の長さ
	const CxMath::Vector3 end    = start + dir * length;
