...
BaseEventObject::BaseEventObject() {
	...
	box->Initialize(CxMath::Vector3(1.0f));			 //< サイズ設定
...
void BaseEventObject::Initialize() {
		...
		//コライダーの色を緑に設定
		collider_->SetColor(CxMath::Vector3(0,1,0));
}
...
void BaseEventObject::AlwaysUpdate([[maybe_unused]] float dt) {
	...
	CxMath::Vector3	   worldPos = worldTransform_.GetWorldPosition();
	CxMath::Quaternion rot		= worldTransform_.rotation;
