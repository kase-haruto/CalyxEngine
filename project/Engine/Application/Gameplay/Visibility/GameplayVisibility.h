...
// AABB の8頂点
	inline void GetCorners(const AABB& b, Vector3 out[8]) {
		const Vector3& mn = b.min_;
		const Vector3& mx = b.max_;
	...
	inline Matrix4x4 GetViewProj(const Camera3d* cam) {
	...
	inline bool IsAabbOnScreenNdc(const Camera3d* cam, const AABB& worldAabb, float padNdc = 0.05f) {
		if (!cam) return true;
		const Matrix4x4 vp = GetViewProj(cam);

		Vector3 corners[8]; GetCorners(worldAabb, corners);
		...
		for (int i = 0; i < 8; ++i) {
			...
			const Vector4 cp = vp * Vector4(p.x, p.y, p.z, 1.0f);
	...
	// 射程チェック（距離の二乗で比較）
	inline bool InEngageRangeSq(const Vector3& from, const Vector3& to, float maxDist) {
