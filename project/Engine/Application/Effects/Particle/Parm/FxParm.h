...
private:
	static T getDefault() { if constexpr(std::is_same_v<T,Vector3>) { return Vector3(1.0f,1.0f,1.0f); } else if constexpr(std::is_arithmetic_v<T>) { return T(1); } else { return T{}; } }
	static T getHalf(const T& v) { if constexpr(std::is_same_v<T,Vector3>) { return v * 0.5f; } else if constexpr(std::is_arithmetic_v<T>) { return v * T(0.5f); } else { return T{}; } }
