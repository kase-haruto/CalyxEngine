...
std::unique_ptr<BaseFxModule> CreateFromConfig(const BaseModuleConfig& config) {
	...
	if (auto* c = dynamic_cast<const TextureSheetAnimationConfig*>(&config)) {
		auto m = std::make_unique<TextureSheetAnimationModule>(c->name);
		m->SetEnabled(c->enabled);
...
std::unique_ptr<BaseModuleConfig> CreateConfigFromModule(const BaseFxModule& module) {
	...
	// TextureSheetAnimation
	if (auto* m = dynamic_cast<const TextureSheetAnimationModule*>(&module)) {
...
//------------------------------------------------------------------------------
std::unique_ptr<BaseFxModule> CreateByName(const std::string& typeName) {
	if (typeName == "GravityModule") {
		return std::make_unique<GravityModule>("GravityModule");
	}
	if (typeName == "SizeOverLifetimeModule") {
		return std::make_unique<SizeOverLiftimeModule>("SizeOverLifetimeModule");
	}
