...
FxModuleContainer::FxModuleContainer(
    const std::vector<std::unique_ptr<BaseModuleConfig>>& moduleConfigs)
{
...
void FxModuleContainer::RemoveModule(const std::string& displayName)
{
    modules_.erase(
        std::remove_if(modules_.begin(), modules_.end(),
            [&](const std::unique_ptr<BaseFxModule>& m){
                return m->GetName() == displayName;
            }),
        modules_.end()
    );
}
...
void FxModuleContainer::ApplyConfigs(
    const std::vector<std::unique_ptr<BaseModuleConfig>>& configs)
{
    modules_.clear();
...
// =============================================================
std::vector<std::unique_ptr<BaseModuleConfig>>
FxModuleContainer::ExtractConfigs() const
{
    ...
    for (const auto& mod : modules_)
    {
        ...
        cfg->name = mod->GetName(); // typeName を入れる
