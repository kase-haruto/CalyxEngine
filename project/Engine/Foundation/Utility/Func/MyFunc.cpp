...
CxMath::Matrix4x4 MakeOrthographicMatrix(float l, float t, float r, float b, float nearClip, float farClip) {
...
ModelData LoadObjFile(const std::string& directoryPath, const std::string& filename) {
	...
	for (uint32_t boneIndex = 0; boneIndex < mesh->mNumBones; ++boneIndex) {
		...
		CxMath::Matrix4x4 inverseBindPoseMatrix = CxMath::MakeAffineMatrix(
...
MaterialData LoadMaterialTemplateFile(const std::string& directoryPath, const std::string& filename) {
	...
	while (std::getline(file, line)) {
		...
		if (identifer == "map_Kd") {// ファイル名
			...
			CxMath::Vector3 scale = { 1.0f,1.0f,1.0f };
			CxMath::Vector3 offset = { 0.0f,0.0f,0.0f };
			CxMath::Vector3 translate = { 0.0f,0.0f,0.0f };
...
bool IsCollision(const AABB& aabb, const CxMath::Vector3& point) {
...
int32_t CreateJoint(const Node& node, const std::optional<int32_t>& parent, std::vector<Joint>& joints) {
	...
	joint.skeletonSpaceMatrix = CxMath::Matrix4x4::MakeIdentity();
...
Node ConvertAssimpNode(const aiNode* node) {
	...
	result.localMatrix =
		CxMath::MakeAffineMatrix(result.transform.scale, result.transform.rotate, result.transform.translate);
...
SkinCluster CreateSkinCluster(const Microsoft::WRL::ComPtr<ID3D12Device>& device,
	...
	std::generate(
		...
		[] (){ return CxMath::Matrix4x4::MakeIdentity(); }
	);
...
CxMath::Matrix4x4 MakeYAxisBillboard(const CxMath::Matrix4x4& cameraMatrix) {
	CxMath::Vector3 camZ = { cameraMatrix.m[0][2], 0.0f, cameraMatrix.m[2][2] };
	camZ = camZ.Normalize();
	CxMath::Vector3 camX = CxMath::Vector3::Cross({ 0, 1, 0 }, camZ).Normalize();
	CxMath::Vector3 camY = CxMath::Vector3::Cross(camZ, camX);
	CxMath::Vector3 cam = (camX, camY, camZ);
	return CxMath::MakeAffineMatrix(CxMath::Vector3::One(), cam, {});
}

CxMath::Matrix4x4 MakeXAxisBillboard(const CxMath::Matrix4x4& cameraMatrix) {
	CxMath::Vector3 camZ = { 0.0f, cameraMatrix.m[1][2], cameraMatrix.m[2][2] };
	camZ = camZ.Normalize();
	CxMath::Vector3 camY = CxMath::Vector3::Cross(camZ, { 1, 0, 0 }).Normalize();
	CxMath::Vector3 camX = CxMath::Vector3::Cross(camY, camZ);
	CxMath::Vector3 cam = (camX, camY, camZ);
	return CxMath::MakeAffineMatrix(CxMath::Vector3::One(), cam, {});
}

CxMath::Matrix4x4 MakeZAxisBillboard(const CxMath::Matrix4x4& cameraMatrix) {
	CxMath::Vector3 camY = { cameraMatrix.m[0][1], cameraMatrix.m[1][1], 0.0f };
	camY = camY.Normalize();
	CxMath::Vector3 camX = CxMath::Vector3::Cross(camY, { 0, 0, 1 }).Normalize();
	CxMath::Vector3 camZ = CxMath::Vector3::Cross(camX, camY);
	CxMath::Vector3 cam = (camX, camY, camZ);
	return CxMath::MakeAffineMatrix(CxMath::Vector3::One(), cam, {});
}