...
namespace Dx{
	...
	[[nodiscard]] D3D12_CPU_DESCRIPTOR_HANDLE
		GetCPUDescriptorHandle(ID3D12DescriptorHeap* heap, uint32_t descriptorSize, uint32_t index) noexcept;


	[[nodiscard]] D3D12_GPU_DESCRIPTOR_HANDLE
		GetGPUDescriptorHandle(ID3D12DescriptorHeap* heap, uint32_t descriptorSize, uint32_t index) noexcept;


	// Optional helpers migrated from DxFunc
