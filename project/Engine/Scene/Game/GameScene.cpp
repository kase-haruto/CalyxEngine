...
void GameScene::Initialize() {
	...
	scoreDigits_ = std::make_unique<NumbersSprite>();
	scoreDigits_->Initialize(/*pos*/ {kWindowWidth * 0.5f,kWindowHeight * 0.5f},
	                                 /*size*/ {64.0f,64.0f});
	scoreDigits_->SetAlign(NumbersSprite::DigitsAlign::Right);
}
...
void GameScene::Update([[maybe_unused]] float dt) {
	...
	// スコア更新
	if(score_) score_->Update();
	scoreDigits_->SetValue(score_->GetTotal());
	scoreDigits_->Update();
...
void GameScene::Draw(ID3D12GraphicsCommandList* cmdList,
	...
	for (auto* sp : scoreDigits_->GetSpritesRaw()) {
