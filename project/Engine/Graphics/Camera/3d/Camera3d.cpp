...
void ExtractBasisNoScale(const CxMath::Matrix4x4& M,CxMath::Vector3& R,CxMath::Vector3& U,CxMath::Vector3& F) {
	R = CxMath::Vector3(M.m[0][0],M.m[0][1],M.m[0][2]).Normalize(); // +X
	U = CxMath::Vector3(M.m[1][0],M.m[1][1],M.m[1][2]).Normalize(); // +Y
	F = CxMath::Vector3(M.m[2][0],M.m[2][1],M.m[2][2]).Normalize(); // +Z (LH: forward)
}

// 位置のアフィン変換（点）
CxMath::Vector3 TransformPoint(const CxMath::Matrix4x4& M,const CxMath::Vector3& p) {
	return {
...
CxMath::Vector3 TransformDirection(const CxMath::Matrix4x4& M,const CxMath::Vector3& v) {
	return {
...
CxMath::Matrix4x4 OrthonormalizeRotation(const CxMath::Matrix4x4& M) {
	CxMath::Vector3 R,U,F;
	ExtractBasisNoScale(M,R,U,F);
	// 再直交化（Gram-Schmidt 簡易）
	R = R.Normalize();
	U = (U - R * CxMath::Vector3::Dot(R,U)).Normalize();
	F = CxMath::Vector3::Cross(R,U).Normalize(); // LHなら R×U=F でOK
...
CxMath::Vector3 Camera3d::SmoothDampVec(const CxMath::Vector3& current,const CxMath::Vector3& target,
								CxMath::Vector3&       vel,float              smoothTime,float dt) {
	...
	CxMath::Vector3 change = current - target;
	CxMath::Vector3 temp   = (vel + change * omega) * dt;
...
void Camera3d::UpdateFollow(float dt) {
	...
	//  ターゲットのワールド行列から基底を抽出
	const CxMath::Matrix4x4& Tw = follow_.target->matrix.world;
	CxMath::Vector3          Tr{Tw.m[3][0],Tw.m[3][1],Tw.m[3][2]}; // target world pos
	CxMath::Vector3          R,U,F;
	ExtractBasisNoScale(Tw,R,U,F); // world right/up/forward

	//  望ましい「カメラのワールド位置」
	CxMath::Vector3 desiredWorldPos =
		Tr
	...
	//  位置スムージングはワールドで
	//    （posVel もワールド速度として保持）
	CxMath::Vector3 curWorldPos = TransformPoint(worldTransform_.matrix.world,{0,0,0}); // 現在のワールド位置
	...
	CxMath::Matrix4x4  targetRotM   = OrthonormalizeRotation(Tw);
	CxMath::Quaternion targetWorldQ = CxMath::Quaternion::FromMatrix(targetRotM);

	//    俯角はワールドの Right 軸（R）回り
	CxMath::Quaternion extraPitch = CxMath::Quaternion::MakeRotateAxisQuaternion(
		R,CxMath::ToRadians(follow_.extraPitchDeg));
	CxMath::Quaternion desiredWorldQ = extraPitch * targetWorldQ;

	//    現在のワールド回転を取得
	CxMath::Matrix4x4  CwRotM    = OrthonormalizeRotation(worldTransform_.matrix.world);
	CxMath::Quaternion curWorldQ = CxMath::Quaternion::FromMatrix(CwRotM);

	//    ワールド回転を指数補間
	float      a         = ExpLerpAlpha(dt,follow_.rotTimeConstant);
	CxMath::Quaternion newWorldQ = CxMath::Quaternion::Slerp(curWorldQ,desiredWorldQ,a);

	// 親のローカルに戻してセット
	if(worldTransform_.parent) {
		CxMath::Matrix4x4 Pw    = worldTransform_.parent->matrix.world;
		CxMath::Matrix4x4 PwInv = CxMath::Matrix4x4::Inverse(Pw);

		// ローカル位置 = 親^-1 * ワールド位置
		CxMath::Vector3 localPos = TransformPoint(PwInv,curWorldPos);

		// ローカル回転 = 親回転^-1 * ワールド回転
		CxMath::Matrix4x4  parentRotM   = OrthonormalizeRotation(Pw);
		CxMath::Matrix4x4  parentRotInv = CxMath::Matrix4x4::Transpose(parentRotM); // 直交なので転置=逆
		CxMath::Matrix4x4  newWorldRotM = CxMath::Quaternion::ToMatrix(newWorldQ);
		CxMath::Matrix4x4  localRotM    = parentRotInv * newWorldRotM;
		CxMath::Quaternion localQ       = CxMath::Quaternion::FromMatrix(localRotM);
