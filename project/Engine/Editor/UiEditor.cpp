...
void UIEditor::ShowImGuiInterface(){
	...
	// スプライトの生成用の入力欄
	static CalyxMath::Vector2 newPosition = {400.0f, 0.0f}; // デフォルトの位置
	static CalyxMath::Vector2 newSize = {64.0f, 64.0f};     // デフォルトのサイズ
	...
	if (ImGui::CollapsingHeader("Sprites List", ImGuiTreeNodeFlags_DefaultOpen)){
		...
		for (auto& sprite : sprites_){
			...
			if (ImGui::CollapsingHeader(spriteLabel.c_str(), ImGuiTreeNodeFlags_None)){
				// スプライトのプロパティ編集
				CalyxMath::Vector2 position = sprite->GetPosition();
				float rotate = sprite->GetRotation();
				CalyxMath::Vector2 size = sprite->GetSize();
...
void UIEditor::AddSprite(const std::string& textureName, const CalyxMath::Vector2& position, const CalyxMath::Vector2& size){
