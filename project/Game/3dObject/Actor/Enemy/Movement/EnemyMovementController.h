...
private:
	...
	CxMath::Vector3 camAnchor_ = {0, 0, 40};

	// Exit
	bool	exitPrepared_  = false;
	CxMath::Vector3 exitDirLocal_  = {0, 0, 0};
	...
	CxMath::Vector3					  formationOffset_ = {0, 0, 0};
	float					  formationPhase_  = 0.0f; // 個体差の揺れ用

	// dissolve 用の速度ベクトル
	CxMath::Vector3 scatterVelocity_ = {0, 0, 0};
	int		formationSize_	 = 1;

	int formationIndex_ = 0;

	// 侵入用
	CxMath::Vector3 entranceStart_	= {0, 0, 0}; // 開始位置（画面外）
	CxMath::Vector3 entranceTarget_ = {0, 0, 0}; // 合流目標（編隊オフセット位置）
