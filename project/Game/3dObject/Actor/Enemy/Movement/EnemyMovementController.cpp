...
namespace {
void ClampToFrustum(CxMath::Vector3& p, float fovY, float aspect, float margin) {
...
CxMath::Vector3 ComputeDissolveDirection(int index, DissolvePattern pattern) {
	switch(pattern) {
	...
	case DissolvePattern::FourWay:
		switch(index % 4) {
		case 0:
			return CxMath::Vector3(-1, +0.5f, 0.0f).Normalize();
		case 1:
			return CxMath::Vector3(+1, +0.5f, 0.0f).Normalize();
		case 2:
			return CxMath::Vector3(-1, -0.5f, 0.0f).Normalize();
		...
		return CxMath::Vector3(0, 0, 1).Normalize();
	...
	case DissolvePattern::Circle: {
		...
		return CxMath::Vector3(std::cos(angle), std::sin(angle), 0.0f).Normalize();
	}
...
void EnemyMovementController::StartStay(float duration) {
	...
	camAnchor_ = CxMath::Vector3(0, 0, 80);
}
...
void EnemyMovementController::UpdateCameraDrift([[maybe_unused]] float dt) {
	...
	CxMath::Vector3 p = camAnchor_ + CxMath::Vector3(dx, dy, dz);
...
void EnemyMovementController::StartDissolve(int index, DissolvePattern pattern) {
	...
	CxMath::Vector3 dir = ComputeDissolveDirection(index, pattern);
...
void EnemyMovementController::UpdateExit(float dt) {
	...
	// 初回：外へ向かう方向を決める
	if(!exitPrepared_) {
		CxMath::Vector3 p = owner_->GetWorldTransform().translation;
		...
		CxMath::Vector3 target = {edgeX, edgeY, p.z};
		CxMath::Vector3 dir	   = (target - p).Normalize();
	...
	// 移動
	CxMath::Vector3 newPos = owner_->GetWorldTransform().translation + exitDirLocal_ * exitSpeed_ * dt;
...
bool EnemyMovementController::CheckExitFinished() const {
	auto		   cam = CameraManager::GetMain3dShared();
	const CxMath::Vector3& p   = owner_->GetWorldTransform().translation;
...
void EnemyMovementController::StartEntranceToFormation(EnemyFormationController* formation, const CxMath::Vector3& formationOffset, const CxMath::Vector3& entranceStartWorld) {
...
void EnemyMovementController::UpdateEntrance(float dt) {
	...
	CxMath::Vector3 pos = CxMath::Vector3::Lerp(entranceStart_, entranceTarget_, ease);
...
void EnemyMovementController::UpdateFormation(float /*dt*/) {
	...
	CxMath::Vector3 leaderPos = formation_->GetPosition();
	CxMath::Vector3 off		  = formationOffset_;
	...
	CxMath::Vector3 localPos = leaderPos + off;
...
void EnemyMovementController::LookAtPlayer() {
	if(playerTf_) {
		CxMath::Vector3 targetWorld = playerTf_->GetWorldPosition();
		CxMath::Vector3 myWorld		= owner_->GetWorldTransform().GetWorldPosition();
		WorldTransform& worldTransform_ = owner_->GetWorldTransform();
		CxMath::Vector3 dir;
		if(worldTransform_.parent) {
			CxMath::Matrix4x4 invParent	  = CxMath::Matrix4x4::Inverse(worldTransform_.parent->matrix.world);
			CxMath::Vector3	  targetLocal = CxMath::Vector3::Transform(targetWorld, invParent);
			CxMath::Vector3	  myLocal	  = CxMath::Vector3::Transform(myWorld, invParent);
		...
		if(dir.LengthSquared() > 1e-12f) {
			...
			worldTransform_.rotation = CxMath::Quaternion::MakeRotateY(yaw) * CxMath::Quaternion::MakeRotateX(pitch);
		}
	}
}
