...
namespace{
	...
	// 今は「位置デルタ」を AddMoveRequest で渡すので false にしておくか、
	// 分岐自体をなくしてもいい
	constexpr bool kMoveByTakesVelocity = false;

	// 1秒間無敵（※ここでは使っていないが、将来のために残すならそのまま）
...
void PlayerDodgeMotion::ApplySpinAndCurve(float dt){
	...
	/* ---------- 回転：絶対駆動（必ず回り切る） ---------- */
	{
		const float spinProgress = (g >= 0.0f) ? CalyxEase::EaseInCubic(g) : 0.0f;
...
void PlayerDodgeMotion::ApplyProceduralPose(float dt){
	...
	if      (st == DodgeState::Startup){  sinkTarget = kSinkStartup; sinkHz = kSinkHzStartup; }
	else if (st == DodgeState::IFrame){   sinkTarget = kSinkIFrame;  sinkHz = kSinkHzIFrame; }
	else if (st == DodgeState::Recovery){ sinkTarget = kSinkRecover; sinkHz = kSinkHzRecover; }
...
bool PlayerDodgeMotion::IsDodging(DodgeState s) {
	return s == DodgeState::Startup
		|| s == DodgeState::IFrame
		|| s == DodgeState::Recovery;
}