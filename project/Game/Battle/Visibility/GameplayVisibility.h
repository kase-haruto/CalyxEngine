...
namespace CalyxUtil {
	namespace GameplayVisibility {
		...
		// AABB の8頂点
		inline void GetCorners(const AABB& b, CalyxMath::Vector3 out[8]) {
			const CalyxMath::Vector3& mn = b.min_;
			const CalyxMath::Vector3& mx = b.max_;
		...
		inline CalyxMath::Matrix4x4 GetViewProj(const Camera3d* cam) {
		...
		inline bool IsAabbOnScreenNdc(const Camera3d* cam, const AABB& worldAabb, float padNdc = 0.05f) {
			if(!cam) return true;
			const CalyxMath::Matrix4x4 vp = GetViewProj(cam);

			CalyxMath::Vector3 corners[8];
			GetCorners(worldAabb, corners);

			const float nx = 1.0f - padNdc; // [-nx, +nx] に入ればOK
			const float ny = 1.0f - padNdc;

			for(int i = 0; i < 8; ++i) {
				const CalyxMath::Vector3& p	 = corners[i];
				const CalyxMath::Vector4  cp = vp * CalyxMath::Vector4(p.x, p.y, p.z, 1.0f);
		...
		// 射程チェック（距離の二乗で比較）
		inline bool InEngageRangeSq(const CalyxMath::Vector3& from, const CalyxMath::Vector3& to, float maxDist) {
			const CalyxMath::Vector3 d	= to - from;
			const float				 d2 = d.x * d.x + d.y * d.y + d.z * d.z;
			return d2 <= (maxDist * maxDist);
		}

	} // namespace CalyxUtil::GameplayVisibility
}
