...
void HpGauge::SetHp(float hp) {
	...
	// ここでは「減ったときだけ揺らす」例：
	if(currentHp_ < blueRatio_ * maxHp_) { shakeTimer_ = 0.3f; }
}
...
void HpGauge::ShowGui(const std::string& label) {
	ImGui::Text("currentHp : %.1f",currentHp_);
	const std::string transformLabel = label + "transform";
	transform_.ShowImGui(transformLabel);
}
