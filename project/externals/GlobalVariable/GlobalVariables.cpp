...
void GlobalVariables::SaveFile(const std::string& groupName) {
	...
	for (auto itItem = itGroup->second.items.begin(); itItem != itGroup->second.items.end(); ++itItem) {
		...
		} else if (std::holds_alternative<CxMath::Vector3>(item.value)) {
			CxMath::Vector3 value = std::get<CxMath::Vector3>(item.value);
...
void GlobalVariables::LoadFile(const std::string& groupName) {
	...
	for (auto itItem = itGroup->begin(); itItem != itGroup->end(); ++itItem) {
		...
		if (itItem->is_number_integer()) {
			...
			CxMath::Vector3 value = { itItem->at(0), itItem->at(1), itItem->at(2) };
			SetValue(groupName, itemName, value);
...
void GlobalVariables::ShowSlider(const std::string& itemName, CxMath::Vector3& value) {
